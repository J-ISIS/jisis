 ...
 *
 */
//import org.unesco.openidams.model.distributed.DistributedTableDataSource;
//import org.unesco.openidams.model.distributed.DistributedTableDescription;
...
public class DistributedTableClientCache {
     ...
     protected static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(DistributedTableClientCache.class);
    ...
    */
   public DistributedTableClientCache(int chunkSize, int maximumCacheSize,
   ...
   private void ensureRowCached(int rowIndex) {
      ...
      if (!isRowCached(rowIndex)) {
         ...
         // FIND OUT THE CHUNK RANGE
         fromIndex_ = 0;
         toIndex_ = chunkSize_;

         // Find out in which range the requested row should fit
         while (toIndex_ < tableDescription_.getRowCount()) {
            if (rowIndex >= fromIndex_ && rowIndex < toIndex_) {
               break;
            }
            fromIndex_ = toIndex_;
            toIndex_ = fromIndex_ + chunkSize_;
            
         }
         if (toIndex_ >= tableDescription_.getRowCount()) {
             if (tableDataSource_.isZeroBased()) {
                  toIndex_ = tableDescription_.getRowCount();
             } else {
         ...
         System.out.println("rowIndex="+rowIndex+" fromIndex_="+fromIndex_+" toIndex_="+toIndex_);
...
//         }
   ...
   public Object[] retrieveRowFromCache(int rowIndex) {
      ...
      Object[] obj = null;
      try {
         obj = (Object[]) data_[index];
      } catch (Exception ex) {
          LOGGER.error("Error when Retrieving_data rowIndex: [{}]",rowIndex,ex);
          LOGGER.error("Error when Retrieving_data index: [{}]",index,ex);
      }
