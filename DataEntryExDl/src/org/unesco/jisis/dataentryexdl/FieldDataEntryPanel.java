...
import org.unesco.jisis.dataentryex.HighlightRenderer;
import org.unesco.jisis.dataentryex.ICellHighlight;
 ...
 * @author jcd
 */
public class FieldDataEntryPanel extends javax.swing.JPanel implements CellEditorListener {
    ...
    private WorksheetField wksField_ = null;
    private FieldDefinitionTable fdt_ = null;
    private Outline outline_;
    private FieldDataEntryTreeModel treeModel_;
    private OutlineModel outlineModel_;
    private FieldDataEntryRowModel rowModel_;
    private IRecord record_ = null;
    private IDatabase db_ = null;
      private DataEntryTopComponent topComponent_ = null;
     ...
     */
    private FieldDataEntryPanel() {
        initComponents();

    }

    public FieldDataEntryPanel(DataEntryTopComponent topComponent, IDatabase db, WorksheetField wksField,
            IRecord record,
        List<PickListData> pickListDataList, List<ValidationData> validationDataList) {
        topComponent_ = topComponent;
        db_ = db;
        wksField_ = wksField;
        wksName_ = wksField.getDescription();
       ...
       if (record == null) {
          treeModel_ = FieldDataEntryTreeModel.makeTreeModelForEmptyField(db, wksField,
                  pickListDataList_, validationDataList_);
       } else {
          treeModel_ = FieldDataEntryTreeModel.makeTreeModelForField(db, wksField,
                  record, pickListDataList, validationDataList, true);
       }

       rowModel_ = new FieldDataEntryRowModel();
...
//                outline.setSelectionForeground(SheetCell.getNoFocusSelectionForeground());
        ...
        outline_.expandPath(treeModel_.treePathToLast());
        ...
        */
        ...
        Action nextRowAction = new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedCol != FieldDataEntryRowModel.DATA_COLUMN_INDEX) {
...
//            outline_.scrollRectToVisible(rect);
        ...
        Action pickListAction = new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (column != FieldDataEntryRowModel.DATA_COLUMN_INDEX) {
                    // Return if we are not on data column
                    return;
                }
                if (!pickList()) {
                ...
                processPickList();
                outline_.setColumnSelectionInterval(FieldDataEntryRowModel.DATA_COLUMN_INDEX,
                        FieldDataEntryRowModel.DATA_COLUMN_INDEX);
    ...
    private void buildPopupMenu() {
        ...
        Action addOccurrenceAction = new AbstractAction("Add Occurrence") {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                WorksheetDef.WorksheetField worksheetField = wksField_;
        ...
        Action deleteSubfieldAction = new AbstractAction("Delete Subfield Occurrence") {
            ...
            // If it remains only 1 occurrence, we cannot delete this
                ...
                if (nSubfieldOccurrences == 1) {
                    ...
                    // Be sure to update the field content
                    rowModel_.setValueFor(node, FieldDataEntryRowModel.DATA_COLUMN_INDEX, "");
...
//            tc.setCellEditor(textEditor);
      ...
      void setField(IRecord record, boolean showEmptyFields) {
        ...
        record_ = record;
        treeModel_ = FieldDataEntryTreeModel.makeTreeModelForField(db_, wksField_, record,
...
//      });
    ...
    void newRecord() {
        record_ = null;
        treeModel_ = FieldDataEntryTreeModel.makeTreeModelForEmptyField(db_, wksField_,
            pickListDataList_, validationDataList_);
...
//      });
    ...
    public void changeWorksheet(WorksheetDef wks, IRecord record, boolean showEmptyFields,
        List<PickListData> pickListDataList, List<ValidationData> validationDataList) {
        WorksheetDef wks_ = wks;
        wksName_ = wks.getName();
        record_ = record;
        pickListDataList_ = pickListDataList;
        validationDataList_ = validationDataList;
        treeModel_ = FieldDataEntryTreeModel.makeTreeModelForField(db_, wksField_, record_,
            pickListDataList_, validationDataList_, showEmptyFields);
    ...
    /**
     * Build the field data value from the field occurrence data. Please note that
     * a non repeatable field has ONE occurrence and that the tag node data is
     * formatted with the subfield delimiters and data.
     * @return 
     */
   public Object getFieldValue() {
      ...
      return (fieldValue != null) ? fieldValue : fieldValues;
   }
    /**
    ...
    class PopupTrigger extends MouseAdapter {
        ...
        public void mouseReleased(MouseEvent e) {
            ...
            if (e.getID() == MouseEvent.MOUSE_RELEASED && e.getButton() == MouseEvent.BUTTON1) {
                ...
                if (row == -1 || col != FieldDataEntryRowModel.PICKLIST_COLUMN_INDEX+1) {
                    return;
                }
