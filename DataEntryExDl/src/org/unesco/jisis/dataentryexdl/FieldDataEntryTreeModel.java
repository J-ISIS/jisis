 ...
 * @author jcd
 */
public class FieldDataEntryTreeModel extends AbstractTreeModel {
   ...
   private List<ValidationData> validationDataList_;
    ...
    private FieldDataEntryTreeModel(IDatabase db, WorksheetField wksField,
        ...
        buildTreeModelForEmptyField(wksField);

    }

   private FieldDataEntryTreeModel(IDatabase db, WorksheetField wksField, IRecord record, 
      ...
      buildTreeModelForField(wksField, record);
   }

     //-----------------------------------------------------------------
   // Public static factory methods
   //-----------------------------------------------------------------
    public static FieldDataEntryTreeModel makeTreeModelForEmptyField(IDatabase db, WorksheetField wksField,
            List<PickListData>  pickListDataList, List<ValidationData> validationDataList) {
        FieldDataEntryTreeModel treeModel = new FieldDataEntryTreeModel(db, wksField,
                pickListDataList, validationDataList);
        return treeModel;
    }

   public static FieldDataEntryTreeModel makeTreeModelForField(IDatabase db, WorksheetField wksField, IRecord record, 
           List<PickListData>  pickListDataList, List<ValidationData> validationDataList, boolean showEmptyFields) {
      FieldDataEntryTreeModel treeModel = new FieldDataEntryTreeModel(db, wksField, record, 
              pickListDataList, validationDataList, showEmptyFields);
      return treeModel;
   }
   

   private void buildTreeModelForField(WorksheetField wksField, IRecord record) {
      ...
      rootNode = new DataEntryNode(wksField.getDescription());

      rootNode.setUserObject(nameMap);

      IField fld = null;
      try {
         fld = record.getField(wksField.getTag());
   ...
   private DataEntryNode[] buildTreelNodesForWksField(WorksheetField wksField,
      ...
      try {
          ...
          */
          PickListData fieldPickListData = null;
          ...
          if (fieldPickListData != null) {
              noTyping = fieldPickListData.isNoType();
          }
         ...
         for (int i = 0; i < nOccurrences; i++) {
            ...
            if (fieldPickListData != null) fieldMap.put("pickList", fieldPickListData);
            ...
            for (int j = 0; j < nWksSubfields; j++) {
               ...
               PickListData subfieldPickListData = null;
               Character wksSubfieldCode = wksSubfield.getSubfieldCode().charAt(1);
               for (PickListData pickList : pickListDataList_) {
                  String subfieldCode = pickList.getSubfieldCode();
                  if (subfieldCode == null || subfieldCode.length() == 0) {
                      continue;
                  }
                  Character subfieldCodeChar = subfieldCode.charAt(0);
                  if (subfieldCode.startsWith("$")) {
                     subfieldCodeChar = subfieldCode.charAt(1);
                  }
                  if (Integer.valueOf(pickList.getTag()) == wksSubfield.getTag()
                          && wksSubfieldCode.equals(subfieldCodeChar)) {
                     subfieldPickListData = pickList;
                     break;
                  }
               }
               if (subfieldPickListData != null) {
                  subfieldMap.put("pickList", subfieldPickListData);
               }
                validationData = null;
                for (ValidationData validation : validationDataList_) {
    ...
    private void buildTreeModelForEmptyField(WorksheetField wksField) {
      Hashtable<String, Object> nameMap = new Hashtable<String, Object>();
      nameMap.put("type", "rootNode");
...
//         }
   ...
   */
   ...
   public TreePath treePathToLast() {
      
      TreePath treePath = new TreePath(rootNode.getLastLeaf().getPath());
      return treePath;
   }
}
