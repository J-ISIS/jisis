...
//        setIcon(Utilities.loadImage(ICON_PATH, true));
    ...
    */
    ...
    private void initComponents() {
        ...
        validationPanel.setLayout(new java.awt.BorderLayout());

        validationTextArea.setColumns(20);
        validationTextArea.setRows(5);
        jScrollPane2.setViewportView(validationTextArea);

        validationPanel.add(jScrollPane2, java.awt.BorderLayout.CENTER);

        tabbedPane.addTab(org.openide.util.NbBundle.getMessage(WebOutputTopComponent.class, "WebOutputTopComponent.validationPanel.TabConstraints.tabTitle"), validationPanel); // NOI18N
...
//      System.out.println(content);
    ...
    private void validate(String content) {
        ...
        p.parseInput(content, "/");
        StringBuilder output = new StringBuilder();
        if ((p.getErrors() != null) && p.getErrors().size() > 0) {
             output.append("HTML generated by the PFT contains errors:\n");
            for (ParseError error : p.getErrors()) {
                int i = error.getPosition();
                String s = content.substring(0, i);
                output.append("[").append(error.getPosition()).append("] ")
                      .append(error.getErrorMessage()).append("\n");
                output.append(s);
                output.append("******\n");
            }
        } else {
            output.append("HTML generated by the PFT has been successfully validated");
        }
        validationTextArea.setText(output.toString());
    }
