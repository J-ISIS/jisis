 ...
 *
 */
public final class MyMarcXmlWriter implements MarcWriter {
     ...
     */
    public MyMarcXmlWriter(OutputStream out) {
        this(out, false);
    }
     ...
     * @throws MarcException
     */
    public MyMarcXmlWriter(OutputStream out, boolean indent) {
     ...
     * @throws MarcException
     */
    public MyMarcXmlWriter(OutputStream out, String encoding) {
     ...
     * @throws MarcException
     */
    public MyMarcXmlWriter(OutputStream out, String encoding, boolean indent) {
        if (out == null) {
     ...
     * @throws SAXException
     */
    public MyMarcXmlWriter(Result result) {
        if (result == null)
     ...
     * @throws SAXException
     */
    public MyMarcXmlWriter(Result result, String stylesheetUrl) {
     ...
     * @param stylesheet
     */
    public MyMarcXmlWriter(Result result, Source stylesheet) {
        if (stylesheet == null)
    ...
    protected void toXml(Record record) throws SAXException {
        ...
        while (i.hasNext()) {
           Object obj = i.next();                  // JCD
            if (obj instanceof DataField) {        // JCD
               DataField  field = (DataField) obj;    // JCD
               atts = new AttributesImpl();
               atts.addAttribute("", "tag", "tag", "CDATA", field.getTag());
               ...
               ControlField field = (ControlField) obj;                        // JCD
               atts = new AttributesImpl();                                    // JCD
               atts.addAttribute("", "tag", "tag", "CDATA", field.getTag());   // JCD

               if (indent)                                                     // JCD
                   handler.ignorableWhitespace("\n    ".toCharArray(), 0, 5);  // JCD

               handler.startElement(Constants.MARCXML_NS_URI, CONTROL_FIELD,   // JCD
                       CONTROL_FIELD, atts);                                   // JCD
               temp = getDataElement(field.getData());                         // JCD
               handler.characters(temp, 0, temp.length);                       // JCD
               handler.endElement(Constants.MARCXML_NS_URI, CONTROL_FIELD,     // JCD
                       CONTROL_FIELD);                                         // JCD
            }                                                                  // JCD
        }  // end while
