...
public class QueryBuilderEditor extends JEditorPane
        implements ActionListener, KeyListener {

   private boolean DEBUG = false;
   private boolean stringIsParsed = false;
   private QueryBuilderTopComponent _queryBuilder;
   private JPopupMenu _sqlTextPopup;
...
//     private DefaultStyledDocument dsDocument   =  (DefaultStyledDocument) getStyledDocument();
    ...
    * exist.
    */
   private /*@ spec_public @*/ boolean docModified = false;
   ...
   // Constructor
   public QueryBuilderEditor(QueryBuilderTopComponent queryBuilder) {
      super();
...
//        StyleConstants.setBold(schema, true);
      StyleConstants.setForeground(schema, new Color(0, 111, 0));

//        StyleConstants.setBold(table, true);

//        StyleConstants.setBold(column, true);
...
//     }
   ...
   JPopupMenu createSqlTextPopup() {
      ...
      parseQueryMenuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderEditor.class, "PARSE_QUERY"));      // NOI18N
      parseQueryMenuItem.addActionListener(this);
      sqlTextPopup.add(parseQueryMenuItem);

      runQueryMenuItem = new JMenuItem(NbBundle.getMessage(QueryBuilderEditor.class, "RUN_QUERY")); // NOI18N
...
//         }
      ...
      if (!text.trim().equals(_lastGoodQuery)) {
         _lastGoodQuery = text;
         _queryChanged = true;
      } else {
         _queryChanged = false;
    ...
    */
   public void saveLastGoodQuery(String query) {
      _lastGoodQuery = query;
   }
    ...
    */
   void clear() {
      this.setQueryText(""); // NOI18N
   }

   /**
    * Resets the text to the last parsed query
    */
   void reset() {
         ...
         SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
               if (dot == mark) {  // no selection
                  _queryBuilder.disableCutCopy();
               } else {
                  _queryBuilder.enableCutCopy();
               }
            }
         });
      }
